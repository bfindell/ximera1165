...
\begin{problem}
	...
	\begin{selectAll}
		\choice{The number of ways to see two threes on four rolls of a 6-sided die.}. % Not sure about this one. 
...
\begin{prompt}
 $\answer{a^4+\answer{4}a^\answer{3}b+\answer{6}a^2b^\answer{2}+\answer{4}ab^\answer{3}+b^4}$. % Oops: Ximera CAS accepts $(a+b)^4$
\end{prompt}
\end{problem}
...
\begin{prompt}
 $\answer{729-1458x+1215x^2-540x^3+135x^4-18x^5+x^6}$  % Oops:  Ximera CAS accepts $(3-x)^6$
\end{prompt}
\end{problem}
...
\begin{prompt}
	$\answer[given]{240240}$ % 14*13*12*11*10
\end{prompt}
\end{problem}
...
\begin{enumerate}
	\item What is the probability that it will snow on both Monday and Tuesday? \begin{prompt} $\answer{0.01}$ \end{prompt} % 0.1*0.1
	\item What is the probability that it will snow on Monday and not snow on Tuesday? \begin{prompt} $\answer{0.09}$ \end{prompt} % 0.1*0.9
	\item What is the probability that it will snow on either Monday or Tuesday (but not both)? \begin{prompt} $\answer{0.18}$ \end{prompt} % 0.1*0.9 + % 0.9*0.1
	\item What is the probability that it will not snow for a whole week (including the weekend)? \begin{prompt} $\answer{0.0944784}$ \end{prompt} % 0.9^5*0.4^2
	\item What is the probability that it will snow on one weekday and not snow on the weekend? \begin{prompt} $\answer{0.052488}$ \end{prompt}  % 5*0.9^4*.1*.4^2
	\item What is the probability that it will snow one day during a week? \begin{prompt} $\answer{0.3359232}$ \end{prompt} % 5*0.9^4*.1*.4^2+0.9^5*0.6*0.4*2
