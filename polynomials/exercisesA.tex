...
\begin{freeResponse}
\begin{hint}
Informally, when dividing a polynomial by a (non-zero) polynomial, we can always find a sensible quotient and remainder (both polynomials).  More formally, given $n(x)$ and a non-zero divisor $d(x)$, we can find $q(x)$ and $r(x)$ such that $\answer{n(x)=d(x)q(x)+r(x)}$ with the degree of $r(x)$ \wordChoice{\choice{greater than}\choice{equal to}\choice[correct]{less than}} the degree of $d(x)$.  
\end{hint}
\end{freeResponse}
\end{problem} 
